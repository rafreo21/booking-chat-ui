...
function Table({ className, ...props }: React.ComponentProps<'table'>) {
  return (
    <div data-slot="table-container" className="relative w-full overflow-x-auto shadow-none">
      <table
        data-slot="table"
...
function TableHead({ className, ...props }: React.ComponentProps<'th'>) {
  return (
    <th
      ...
      className={cn(
        'h-11 px-3 text-left align-middle text-[13px] font-semibold text-foreground [&:has([role=checkbox])]:pr-0',
        className,
      )}
      {...props}
    />
