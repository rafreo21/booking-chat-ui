...
/** Email OTP / magic link — same flow as sign-up for Supabase email auth. */
export async function sendRestaurantMagicLinkSignIn(
  email: string,
  ...
  const { error } = await supabase.auth.signInWithOtp({
    email: email.trim(),
    options: { emailRedirectTo: `${magicLinkRedirectOrigin()}/restaurant/auth/callback` },
  })
  if (error) return { error: error.message }
  return undefined
}

export async function sendRestaurantMagicLinkSignUp(
  email: string,
): Promise<{ error: string } | undefined> {
